...
#define CorrectedVector FVector3f
#define CorrectedVector2D FVector2f
#else
#define CorrectedVector FVector
#define CorrectedVector2D FVector2D
#endif
...
public:
	...
	void AddComponent(UTitanWidgetComponent* Component)
	{
		if (Component)
		{
			Components.AddUnique(Component);

			if (TSharedPtr<STitanWorldWidgetScreenLayer> ScreenLayer = ScreenLayerPtr.Pin())
			{
	...
	void RemoveComponent(UTitanWidgetComponent* Component)
	{
		if (Component)
		{
	...
	virtual TSharedRef<SWidget> AsWidget() override
	{
		...
		TSharedRef<STitanWorldWidgetScreenLayer> NewScreenLayer = SNew(STitanWorldWidgetScreenLayer, OwningPlayer);
		ScreenLayerPtr = NewScreenLayer;

		// Add all the pending user widgets to the surface
		for ( TWeakObjectPtr<UTitanWidgetComponent>& WeakComponent : Components )
		{
			if ( UTitanWidgetComponent* Component = WeakComponent.Get() )
			{
				if ( UUserWidget* UserWidget = Component->GetUserWidgetObject() )
				{
...
private:
	...
	TWeakPtr<STitanWorldWidgetScreenLayer> ScreenLayerPtr;
	TArray<TWeakObjectPtr<UTitanWidgetComponent>> Components;
};
...
public:
		...
		if( World.IsValid() /*&& ensure( World->IsGameWorld() )*/ )
		{
			...
			if ( SafeWorld )
			{
				...
				if( TargetPlayer && TargetPlayer->PlayerController )
				{
					...
					{
						if ( UTitanWidgetComponent* WidgetComponent = Cast<UTitanWidgetComponent>(HitComponent) )
						{
	...
	{
		for( TWeakObjectPtr<UTitanWidgetComponent> Component : RegisteredComponents )
		{
			UTitanWidgetComponent* WidgetComponent = Component.Get();
			// Check if visible;
	...
	void RegisterWidgetComponent( UTitanWidgetComponent* InComponent )
	{
		RegisteredComponents.AddUnique( InComponent );
	}

	void UnregisterWidgetComponent( UTitanWidgetComponent* InComponent )
	{
...
private:
	TArray< TWeakObjectPtr<UTitanWidgetComponent> > RegisteredComponents;
...
public:
	...
	FWidget3DSceneProxy( UTitanWidgetComponent* InComponent, ISlate3DRenderer& InRenderer )
			...
			if ( TextureResource )
			{
				if (GeometryMode == EWidgetGeometryMode::Plane)
				{
					...
					for ( int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++ )
					{
						...
						{
							VertexIndices[0] = MeshBuilder.AddVertex(-CorrectedVector(0, U, V ),  CorrectedVector2D(0, 0), CorrectedVector(0, -1, 0), CorrectedVector(0, 0, -1), CorrectedVector(1, 0, 0), FColor::White);
							VertexIndices[1] = MeshBuilder.AddVertex(-CorrectedVector(0, U, VL),  CorrectedVector2D(0, 1), CorrectedVector(0, -1, 0), CorrectedVector(0, 0, -1), CorrectedVector(1, 0, 0), FColor::White);
							VertexIndices[2] = MeshBuilder.AddVertex(-CorrectedVector(0, UL, VL), CorrectedVector2D(1, 1), CorrectedVector(0, -1, 0), CorrectedVector(0, 0, -1), CorrectedVector(1, 0, 0), FColor::White);
							VertexIndices[3] = MeshBuilder.AddVertex(-CorrectedVector(0, UL, V),  CorrectedVector2D(1, 0), CorrectedVector(0, -1, 0), CorrectedVector(0, 0, -1), CorrectedVector(1, 0, 0), FColor::White);
				...
				else
				{
					...
					for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
					{
						...
						if (VisibilityMap & (1 << ViewIndex))
						{
							...
							CorrectedVector LastTangentX;
							CorrectedVector LastTangentY;
							CorrectedVector LastTangentZ;
							...
							for (int32 Segment = 0; Segment < NumSegments; Segment++ )
							{
								...
								const CorrectedVector Vertex0 = -CorrectedVector(X0, PivotOffsetX + Y0, V);
								const CorrectedVector Vertex1 = -CorrectedVector(X0, PivotOffsetX + Y0, VL);
								const CorrectedVector Vertex2 = -CorrectedVector(X1, PivotOffsetX + Y1, VL);
								const CorrectedVector Vertex3 = -CorrectedVector(X1, PivotOffsetX + Y1, V);

								CorrectedVector TangentX = Vertex3 - Vertex0;
								TangentX.Normalize();
								CorrectedVector TangentY = Vertex1 - Vertex0;
								TangentY.Normalize();
								CorrectedVector TangentZ = CorrectedVector::CrossProduct(TangentX, TangentY);
								...
								VertexIndices[0] = MeshBuilder.AddVertex(Vertex0, CorrectedVector2D(U0, 0), LastTangentX, LastTangentY, LastTangentZ, FColor::White);
								VertexIndices[1] = MeshBuilder.AddVertex(Vertex1, CorrectedVector2D(U0, 1), LastTangentX, LastTangentY, LastTangentZ, FColor::White);
								VertexIndices[2] = MeshBuilder.AddVertex(Vertex2, CorrectedVector2D(U1, 1), TangentX, TangentY, TangentZ, FColor::White);
								VertexIndices[3] = MeshBuilder.AddVertex(Vertex3, CorrectedVector2D(U1, 0), TangentX, TangentY, TangentZ, FColor::White);
...
UTitanWidgetComponent::UTitanWidgetComponent( const FObjectInitializer& PCIP )
	: Super( PCIP )
...
void UTitanWidgetComponent::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
bool UTitanWidgetComponent::CanBeInCluster() const
{
	return false;
}

void UTitanWidgetComponent::BeginPlay()
{
...
void UTitanWidgetComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UTitanWidgetComponent::OnLevelRemovedFromWorld(ULevel* InLevel, UWorld* InWorld)
{
...
void UTitanWidgetComponent::SetMaterial(int32 ElementIndex, UMaterialInterface* Material)
{
...
void UTitanWidgetComponent::UpdateMaterialInstance()
{
...
FPrimitiveSceneProxy* UTitanWidgetComponent::CreateSceneProxy()
{
	if (Space == EWidgetSpace::Screen)
	{
		return nullptr;
	...
	public:
		...
		FWidgetBoxProxy(const UTitanWidgetComponent* InComponent)
			: FPrimitiveSceneProxy(InComponent)
...
FBoxSphereBounds UTitanWidgetComponent::CalcBounds(const FTransform & LocalToWorld) const
{
	if ( Space != EWidgetSpace::Screen )
	{
...
UBodySetup* UTitanWidgetComponent::GetBodySetup() 
{
	UpdateBodySetup();
	return BodySetup;
}

FCollisionShape UTitanWidgetComponent::GetCollisionShape(float Inflation) const
{
	if ( Space != EWidgetSpace::Screen )
	{
...
void UTitanWidgetComponent::OnRegister()
{
...
void UTitanWidgetComponent::SetWindowFocusable(bool bInWindowFocusable)
{
...
EVisibility UTitanWidgetComponent::ConvertWindowVisibilityToVisibility(EWindowVisibility visibility)
{
	switch (visibility)
	{
...
void UTitanWidgetComponent::OnWidgetVisibilityChanged(ESlateVisibility InVisibility)
{
...
void UTitanWidgetComponent::SetWindowVisibility(EWindowVisibility InVisibility)
{
	ensure(TickMode == ETickMode::Automatic);
	ensure(Widget);
	ensure(bOnWidgetVisibilityChangedRegistered);
...
void UTitanWidgetComponent::SetTickMode(ETickMode InTickMode)
{
	TickMode = InTickMode;
	SetComponentTickEnabled(InTickMode != ETickMode::Disabled);
}

bool UTitanWidgetComponent::IsWidgetVisible() const
{	
...
bool UTitanWidgetComponent::CanReceiveHardwareInput() const
{
	return bReceiveHardwareInput && GeometryMode == EWidgetGeometryMode::Plane;
}

void UTitanWidgetComponent::RegisterHitTesterWithViewport(TSharedPtr<SViewport> ViewportWidget)
{
#if !UE_SERVER
...
void UTitanWidgetComponent::UnregisterHitTesterWithViewport(TSharedPtr<SViewport> ViewportWidget)
{
#if !UE_SERVER
...
void UTitanWidgetComponent::OnUnregister()
{
#if !UE_SERVER
...
void UTitanWidgetComponent::DestroyComponent(bool bPromoteChildren/*= false*/)
{
...
void UTitanWidgetComponent::ReleaseResources()
{
	if ( Widget )
	{
...
void UTitanWidgetComponent::RegisterWindow()
{
	if ( SlateWindow.IsValid() )
	{
...
void UTitanWidgetComponent::UnregisterWindow()
{
	if ( SlateWindow.IsValid() )
	{
		if ( !CanReceiveHardwareInput() && FSlateApplication::IsInitialized() )
...
void UTitanWidgetComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (!IsRunningDedicatedServer())
	{
		...
		if (Widget && TickMode == ETickMode::Automatic && !IsWidgetVisible())
		{
			...
			if (!bOnWidgetVisibilityChangedRegistered)
			{
				Widget->OnNativeVisibilityChanged.AddUObject(this, &UTitanWidgetComponent::OnWidgetVisibilityChanged);
...
bool UTitanWidgetComponent::ShouldDrawWidget() const
{
...
void UTitanWidgetComponent::DrawWidgetToRenderTarget(float DeltaTime)
{
	if ( GUsingNullRHI )
	{
		return;
...
float UTitanWidgetComponent::ComputeComponentWidth() const
{
	switch (GeometryMode)
	{
		default:
...
TStructOnScope<FActorComponentInstanceData> UTitanWidgetComponent::GetComponentInstanceData() const
{
	return MakeStructOnScope<FActorComponentInstanceData, FTitanWidgetComponentInstanceData>(this);
}

void UTitanWidgetComponent::ApplyComponentInstanceData(FTitanWidgetComponentInstanceData* WidgetInstanceData)
{
...
void UTitanWidgetComponent::GetUsedMaterials(TArray<UMaterialInterface*>& OutMaterials, bool bGetDebugMaterials) const
{
	if (MaterialInstance)
	{
...
bool UTitanWidgetComponent::CanEditChange(const FProperty* InProperty) const
{
	if ( InProperty )
	{
		FString PropertyName = InProperty->GetName();

		if ( PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, GeometryMode) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, TimingPolicy) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, bWindowFocusable) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, WindowVisibility) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, bManuallyRedraw) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, RedrawTime) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, BackgroundColor) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, TintColorAndOpacity) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, OpacityFromTexture) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, BlendMode) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, bIsTwoSided) ||
			 PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, TickWhenOffscreen) )
		{
			return Space != EWidgetSpace::Screen;
		}

		if ( PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, bReceiveHardwareInput) )
		{
			return Space != EWidgetSpace::Screen && GeometryMode == EWidgetGeometryMode::Plane;
		}

		if ( PropertyName == GET_MEMBER_NAME_STRING_CHECKED(UTitanWidgetComponent, CylinderArcAngle) )
		{
...
void UTitanWidgetComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UTitanWidgetComponent::InitWidget()
{
...
void UTitanWidgetComponent::SetOwnerPlayer(ULocalPlayer* LocalPlayer)
{
...
void UTitanWidgetComponent::SetManuallyRedraw(bool bUseManualRedraw)
{
	bManuallyRedraw = bUseManualRedraw;
}

ULocalPlayer* UTitanWidgetComponent::GetOwnerPlayer() const
{
	if (OwnerPlayer)
	{
		return OwnerPlayer;
...
UUserWidget* UTitanWidgetComponent::GetWidget() const
{
	return Widget;
}

void UTitanWidgetComponent::SetWidget(UUserWidget* InWidget)
{
	if (InWidget != nullptr)
	{
...
void UTitanWidgetComponent::SetSlateWidget(const TSharedPtr<SWidget>& InSlateWidget)
{
	if (Widget != nullptr)
	{
		SetWidget(nullptr);
...
void UTitanWidgetComponent::UpdateWidget()
{
	// Don't do any work if Slate is not initialized
	if (FSlateApplication::IsInitialized() &&UKismetSystemLibrary::IsValid(this))
	{
		if ( Space != EWidgetSpace::Screen )
		{
...
void UTitanWidgetComponent::UpdateRenderTarget(FIntPoint DesiredRenderTargetSize)
{
...
void UTitanWidgetComponent::UpdateBodySetup( bool bDrawSizeChanged )
{
	if (Space == EWidgetSpace::Screen)
	{
...
void UTitanWidgetComponent::GetLocalHitLocation(FVector WorldHitLocation, FVector2D& OutLocalWidgetHitLocation) const
{
...
TOptional<float> UTitanWidgetComponent::FindLineSphereIntersection(const FVector& Start, const FVector& Dir, float Radius)
{
...
TTuple<FVector, FVector2D> UTitanWidgetComponent::GetCylinderHitLocation(FVector WorldHitLocation, FVector WorldHitDirection) const
{
	...
	if (bDrawCollisionDebug)
	{
		...
		UKismetSystemLibrary::DrawDebugArrow((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector::ZeroVector), ToWorld.TransformPosition(FVector(50.f, 0, 0)), 2.0f, FLinearColor::Red);
		UKismetSystemLibrary::DrawDebugArrow((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector::ZeroVector), ToWorld.TransformPosition(FVector(0, 50.f, 0)), 2.0f, FLinearColor::Green);
		UKismetSystemLibrary::DrawDebugArrow((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector::ZeroVector), ToWorld.TransformPosition(FVector(0, 0, 50.f)), 2.0f, FLinearColor::Blue);
		...
		UKismetSystemLibrary::DrawDebugCircle((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector::ZeroVector), ToWorld.GetScale3D().X*Radius, 64, FLinearColor::Green,
			0, 1.0f, FVector(0, 1, 0), FVector(1, 0, 0));
		UKismetSystemLibrary::DrawDebugLine((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector(-Apothem, -Radius, 0.0f)), ToWorld.TransformPosition(FVector(-Apothem, +Radius, 0.0f)), FLinearColor::Green);
	}
	...
	if (bDrawCollisionDebug)
	{
		UKismetSystemLibrary::DrawDebugCircle((UTitanWidgetComponent*)(this), ToWorld.TransformPosition(FVector(HitLocation_CircleSpace.X, HitLocation_CircleSpace.Y,0)), 2.0f);
	...
	if (Solution.IsSet())
	{
		...
		if (bDrawCollisionDebug)
		{
			UKismetSystemLibrary::DrawDebugLine((UTitanWidgetComponent*)(this),
				ToWorld.TransformPosition(FVector(HitLocation_CircleSpace.X, HitLocation_CircleSpace.Y, 0.0f)),
...
UUserWidget* UTitanWidgetComponent::GetUserWidgetObject() const
{
	return Widget;
}

UTextureRenderTarget2D* UTitanWidgetComponent::GetRenderTarget() const
{
	return RenderTarget;
}

UMaterialInstanceDynamic* UTitanWidgetComponent::GetMaterialInstance() const
{
	return MaterialInstance;
}

const TSharedPtr<SWidget>& UTitanWidgetComponent::GetSlateWidget() const
{
	return SlateWidget;
}

TArray<FWidgetAndPointer> UTitanWidgetComponent::GetHitWidgetPath(FVector WorldHitLocation, bool bIgnoreEnabledStatus, float CursorRadius)
{
...
TArray<FWidgetAndPointer> UTitanWidgetComponent::GetHitWidgetPath(FVector2D WidgetSpaceHitCoordinate, bool bIgnoreEnabledStatus, float CursorRadius /*= 0.0f*/)
{
...
TSharedPtr<SWindow> UTitanWidgetComponent::GetSlateWindow() const
{
	return SlateWindow;
}

FVector2D UTitanWidgetComponent::GetDrawSize() const
{
	return DrawSize;
}

FVector2D UTitanWidgetComponent::GetCurrentDrawSize() const
{
	return CurrentDrawSize;
}

void UTitanWidgetComponent::SetDrawSize(FVector2D Size)
{
...
void UTitanWidgetComponent::RequestRedraw()
{
	bRedrawRequested = true;
}

void UTitanWidgetComponent::RequestRenderUpdate()
{
	bRedrawRequested = true;
	if (TickMode == ETickMode::Disabled)
	{
...
void UTitanWidgetComponent::SetBlendMode( const EWidgetBlendMode NewBlendMode )
{
...
void UTitanWidgetComponent::SetTwoSided( const bool bWantTwoSided )
{
...
void UTitanWidgetComponent::SetBackgroundColor( const FLinearColor NewBackgroundColor )
{
...
void UTitanWidgetComponent::SetTintColorAndOpacity( const FLinearColor NewTintColorAndOpacity )
{
...
void UTitanWidgetComponent::SetOpacityFromTexture( const float NewOpacityFromTexture )
{
...
TSharedPtr< SWindow > UTitanWidgetComponent::GetVirtualWindow() const
{
	return StaticCastSharedPtr<SWindow>(SlateWindow);
}

UMaterialInterface* UTitanWidgetComponent::GetMaterial(int32 MaterialIndex) const
{
...
int32 UTitanWidgetComponent::GetNumMaterials() const
{
	return FMath::Max<int32>(OverrideMaterials.Num(), 1);
}

void UTitanWidgetComponent::UpdateMaterialInstanceParameters()
{
	if ( MaterialInstance )
	{
		MaterialInstance->SetTextureParameterValue("SlateUI", RenderTarget);
...
void UTitanWidgetComponent::SetWidgetClass(TSubclassOf<UUserWidget> InWidgetClass)
{
