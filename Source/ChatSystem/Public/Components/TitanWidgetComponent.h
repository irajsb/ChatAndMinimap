 ...
 *
 * Titan widget component can stay on screen even when outside viewports (useful for pings and waypoints)
 */
UCLASS(Blueprintable, ClassGroup="UserInterface", hidecategories=(Object,Activation,"Components|Activation",Sockets,Base,Lighting,LOD,Mesh), editinlinenew, meta=(BlueprintSpawnableComponent) )
class CHATSYSTEM_API UTitanWidgetComponent : public UMeshComponent
{
...
public:
	...
	static TOptional<float> FindLineSphereIntersection(const FVector& Start, const FVector& Dir, float Radius);
	...
	void ApplyComponentInstanceData(struct FTitanWidgetComponentInstanceData* ComponentInstanceData);
...
public:
	//angle of out of bounds widget
	UPROPERTY(BlueprintReadOnly,Category="Bounds")
	float OutOfBoundsAngle;
...
USTRUCT()
struct FTitanWidgetComponentInstanceData : public FSceneComponentInstanceData
{
	GENERATED_BODY()
public:
	FTitanWidgetComponentInstanceData()
		: RenderTarget(nullptr)
	{}

	FTitanWidgetComponentInstanceData(const UTitanWidgetComponent* SourceComponent)
		: FSceneComponentInstanceData(SourceComponent)
		, WidgetClass(SourceComponent->GetWidgetClass())
		, RenderTarget(SourceComponent->GetRenderTarget())
	{}
	virtual ~FTitanWidgetComponentInstanceData() = default;
	...
	virtual void ApplyToComponent(UActorComponent* Component, const ECacheApplyPhase CacheApplyPhase) override
	{
		...
		CastChecked<UTitanWidgetComponent>(Component)->ApplyComponentInstanceData(this);
	}
